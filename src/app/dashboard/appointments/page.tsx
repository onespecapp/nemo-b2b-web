...
function AppointmentsPageInner() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      showToast('Error creating slot: ' + error.message, 'error')
    } else {
      setFormData({
        customer_id: '',
        title: '',
  ...
  const handleEditSave = async (e: React.FormEvent) => {
    ...
    if (error) {
      showToast('Error updating slot: ' + error.message, 'error')
    } else {
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <h1 className="font-display text-3xl sm:text-4xl">Test Drives &amp; Service Slots</h1>
        ...
        >
          {showForm ? 'Close form' : 'New slot'}
        </button>
      </div>

      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="flex-1 rounded-full border border-[#0f1f1a]/10 bg-white px-4 py-2 text-sm shadow-sm">
          <label htmlFor="appointment-search" className="sr-only">Search slots</label>
          <input
            id="appointment-search"
            type="search"
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            placeholder="Search by lead, phone, or title..."
      ...
      {showForm && (
        <div className="rounded-3xl border border-[#0f1f1a]/10 bg-white/90 p-6 shadow-sm">
          <h2 className="font-display text-2xl">New slot</h2>
          {customers.length === 0 ? (
            <div className="mt-6 rounded-2xl border border-dashed border-[#0f1f1a]/20 bg-[#f8f5ef] px-6 py-8 text-center">
              <p className="text-sm text-[#0f1f1a]/60">Add leads first before creating slots.</p>
            ...
            <form onSubmit={handleSubmit} className="mt-6 space-y-4">
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  <label htmlFor="new-appointment-customer" className="block text-xs uppercase tracking-[0.2em] text-[#0f1f1a]/60">Lead *</label>
                    ...
                    {customerMenuOpen && (
                      ...
                      >
                        {filteredCustomers.length === 0 ? (
                          <li className="px-4 py-2 text-xs text-[#0f1f1a]/50" role="option" aria-selected={false}>No matching leads</li>
                        ) : (
      ...
      {appointments.length === 0 ? (
        <div className="rounded-3xl border border-dashed border-[#0f1f1a]/20 bg-white/80 p-10 text-center">
          <h3 className="font-display text-2xl">No slots yet</h3>
          <p className="mt-2 text-sm text-[#0f1f1a]/60">Schedule your first test drive or service slot to start tracking outcomes.</p>
          <button
      ...
      <AccessibleModal
        ...
        ariaLabel={`Edit slot: ${editingAppointment?.title || ''}`}
        ...
        {editingAppointment && (
          <>
            <div className="flex items-center justify-between border-b border-[#0f1f1a]/10 bg-white/80 px-8 py-5 backdrop-blur">
              <div>
                <p className="text-xs uppercase tracking-[0.3em] text-[#0f1f1a]/50">Edit slot</p>
            ...
            <div className="max-h-[calc(92vh-210px)] overflow-y-auto px-8 py-6">
              <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
                <div className="rounded-3xl border border-[#0f1f1a]/10 bg-white p-6 shadow-sm">
                  ...
                  {customers.length === 0 ? (
                    <div className="mt-6 rounded-2xl border border-dashed border-[#0f1f1a]/20 bg-[#f8f5ef] px-6 py-8 text-center">
                      <p className="text-sm text-[#0f1f1a]/60">Add leads first before editing slots.</p>
                      <Link href="/dashboard/customers" className="mt-3 inline-flex text-sm font-semibold text-[#0f1f1a]">
                        Go to Customers →
                      </Link>
                    </div>
                    ...
                    <form onSubmit={handleEditSave} className="mt-6 space-y-5">
                      <div className="grid gap-4 sm:grid-cols-2">
                        <div>
                          <label htmlFor="edit-appointment-customer" className="block text-xs uppercase tracking-[0.2em] text-[#0f1f1a]/60">Lead *</label>
                          <div className="relative mt-2">
                            <input
                              ref={editInputRef}
