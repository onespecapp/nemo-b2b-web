...
function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  const navLinks = [
    ...
    { href: '/dashboard/receptionist', label: 'AI Assistant', icon: (
      <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M14.5 6.2a3.5 3.5 0 013.3 3.3M14.5 3a6.5 6.5 0 016.3 6.3" />
      </svg>
    )},
    ...
    { href: '/dashboard/messages', label: 'Leads Inbox', icon: (
      <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    { href: '/dashboard/customers', label: 'Leads', icon: (
      <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  return (
    <div className="min-h-screen bg-[#f8f5ef] text-[#0f1f1a]">
      ...
      <div className="relative">
        <header className="lg:hidden border-b border-[#0f1f1a]/10 bg-white/80 backdrop-blur">
          <div className="flex items-center justify-between px-4 py-4">
            <button
              ...
              className="rounded-xl border border-[#0f1f1a]/10 bg-white px-3 py-2 text-[#0f1f1a] shadow-sm focus:outline-none focus:ring-2 focus:ring-[#f97316]"
            >
              {mobileMenuOpen ? (
        ...
        {mobileMenuOpen && (
          <nav className="lg:hidden border-b border-[#0f1f1a]/10 bg-white/95 px-4 pb-4" aria-label="Mobile menu">
            <div className="space-y-2">
              {navLinks.map((link) => (
                <Link
                  ...
                  className={`flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-medium transition focus:outline-none focus:ring-2 focus:ring-[#f97316] ${
                    pathname === link.href
            ...
            <nav className="space-y-1" aria-label="Main navigation">
              {navLinks.map((link) => (
                <Link
                  ...
                  href={link.href}
                  aria-current={pathname === link.href ? 'page' : undefined}
                  className={`flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-semibold transition focus:outline-none focus:ring-2 focus:ring-[#f97316] ${
                    pathname === link.href
        ...
        <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 border-t border-[#0f1f1a]/10 bg-white/95 backdrop-blur" aria-label="Mobile navigation">
          <div className="grid grid-cols-7 overflow-x-auto">
            {navLinks.map((link) => (
              <Link
                ...
                aria-current={pathname === link.href ? 'page' : undefined}
                className={`flex items-center justify-center min-h-[44px] min-w-[44px] p-3 transition focus:outline-none focus:ring-2 focus:ring-inset focus:ring-[#f97316] ${
                  pathname === link.href
                    ? 'text-[#0f1f1a]'
                    : 'text-[#0f1f1a]/40'
                }`}
