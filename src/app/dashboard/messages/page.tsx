...
export default function MessagesPage() {
  ...
  async function handleCallBack(msg: Message) {
    if (!business || !msg.caller_phone || callingBack) return
    setCallingBack(true)
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_API_URL || 'http://localhost:6001'}/api/outbound-call`,
        {
          ...
          body: JSON.stringify({
            phone: msg.caller_phone,
            business_id: business.id,
            call_purpose: 'follow_up',
            customer_name: msg.caller_name || '',
            context: msg.message || '',
          }),
        }
      )
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <h1 className="font-display text-3xl sm:text-4xl">Leads Inbox</h1>
          <p className="mt-1 text-sm text-[#0f1f1a]/60">Callbacks and follow-ups captured by your AI assistant.</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="py-12 text-center">
            ...
            <p className="mt-1 text-sm text-[#0f1f1a]/40">
              Lead callbacks and call notes will appear here.
            </p>
          </div>
        ) : (
            ...
            <div className="max-h-[calc(90vh-140px)] overflow-y-auto px-6 py-5 space-y-4">
              ...
              <div className="flex justify-end gap-3">
                {selectedMessage.caller_phone && (
                  <button
                    onClick={() => handleCallBack(selectedMessage)}
